	...
	private function _writeData($path, $data, $method = 'PUT', $options = array(), $retries = 0)
	{
		...
		try {
			...
			while (--$retries >= 0) {
				...
				if ($status == 200 && ($jsonData == $return || str_replace('"', '', $jsonData) == str_replace('"', '', $return)))
					break;
				$this->getDI()->get('logger')->warning("Firebase retry: " . $path . ' ' . $status . ' ' . $return . ' should be: ' . $jsonData);
				usleep(250000);
			}
		} catch (Exception $e) {
			$return = null;
