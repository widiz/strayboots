  ...
  public function indexAction()
  {
    ...
    if ($this->player instanceof Players) {
      ...
      if ($id > 0 && !$saudi) {
        ...
        if ($orderHunt->order_id == 2651 && $validEmail) {
          $existingPlayer = null;
          ...
          foreach ($players as $p) {
            if ($p->Team->order_hunt_id == $orderHunt->id)
              $existingPlayer = $p;
          }
          if ($existingPlayer) {
            $action = (int)$this->request->getPost('existing', 'int', 0);
            if ($action === 1) {
              $this->redis->delete(SB_PREFIX . 'ohloc:' . $orderHunt->id . ':' . $existingPlayer->team_id);
              $playerTime = $this->redis->get('pt:' . $existingPlayer->id);
              if ($playerTime > 0) {
                $team = $existingPlayer->Team;
                if ($team->leader == $existingPlayer->id) {
                  $team->activation = date('Y-m-d H:i:s', time() - $playerTime + strtotime($team->activation));
                  $team->save();
                }
              }
              Players::setPlayerLogin($existingPlayer, true);
              $this->view->disable();
              return $this->response->redirect('play');
            } elseif ($action !== 2) {
              define('cacheFileId', 'existing');
              $this->view->request = $this->request->getPost();
              return $this->view->pick('index/existing');
            }
          }
        }
    ...
    $this->view->orderId = false;
    if (is_object($this->orderHunt)) {
