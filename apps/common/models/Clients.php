...
class Clients extends \Phalcon\Mvc\Model
{
	...
	public function initialize()
	{
		...
		$this->hasMany('id', 'Orders', 'client_id', [
			'alias' => 'Orders',
			'foreignKey' => [
				'action' => Relation::ACTION_RESTRICT,
				'message' => 'This client has orders'
			]
		]);
	}
	...
	public function checkLogin($password, $remember = false) {
		if ($this->active && $password == $this->password) {
			Clients::setClientLogin($this, $remember);
			return true;
		}
		return false;
	}

	public static function setClientLogin(Clients $client, $remember = false) {
		$di = Di::getDefault();
		$di->get('session')->set('clientID', (int)$client->id);
		if ($remember) {
			$verifier = md5($client->id . '.strayboots%clients$salt#$%^34564' . $client->email);
			$lifetime = 86400 * 7; // 7 days cookie
			$di->get('redis')->set(SB_PREFIX . 'cverifier:' . $verifier, (int)$client->id, $lifetime);
			$lifetime += time();
			$secure = false;//$di->get('request')->isSecureRequest();
			$di->get('cookies')->set('cid', $client->id, $lifetime, '/', $secure)
								->set('cverifier', $verifier, $lifetime, '/', $secure);
		}
	}
	...
	public static function logout() {
		$di = Di::getDefault();
		$secure = false;//$di->get('request')->isSecureRequest();
		$di->get('cookies')->set('cid', '', 0, '/', $secure)
							->set('cverifier', '', 0, '/', $secure);
		$di->get('session')->remove('clientID');
	}
